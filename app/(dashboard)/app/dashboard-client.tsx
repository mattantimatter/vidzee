...
const STATUS_LABELS: Record<string, { label: string; color: string; pulse?: boolean }> = {
  ...
  uploading: { label: "Uploading", color: "bg-yellow-400", pulse: true },
  tagging: { label: "Analyzing", color: "bg-blue-400", pulse: true },
  storyboard_ready: { label: "Storyboard Ready", color: "bg-blue-500" },
  clips_queued: { label: "Clips Queued", color: "bg-orange-400", pulse: true },
  clips_generating: { label: "Generating Clips", color: "bg-orange-500", pulse: true },
  clips_ready: { label: "Clips Ready", color: "bg-green-400" },
  details_ready: { label: "Details Ready", color: "bg-green-500" },
  render_queued: { label: "Render Queued", color: "bg-purple-400", pulse: true },
  rendering: { label: "Rendering", color: "bg-purple-500", pulse: true },
...
function ProjectCard({ project }: { project: Project }): ReactNode {
  ...
  return (
    <Link href={link} className="block">
      <motion.div
        whileHover={{ y: -3, boxShadow: "0 12px 40px rgba(14,165,233,0.10)" }}
        whileTap={{ scale: 0.975 }}
        transition={{ duration: 0.2, ease }}
        className="group relative border border-neutral-200 dark:border-neutral-700 rounded-2xl p-3.5 sm:p-4 md:p-5 bg-white dark:bg-neutral-800 hover:border-accent/40 dark:hover:border-accent/40 transition-colors cursor-pointer"
      >
        {/* Compact row layout on mobile, original layout on larger screens */}
        <div className="flex items-center gap-3 sm:block">
          {/* Icon */}
          <motion.div
            className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-accent/10 flex items-center justify-center shrink-0 sm:mb-4 group-hover:bg-accent/15 transition-colors"
          >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between gap-2 sm:mb-1">
              <h3 className="font-semibold text-neutral-900 dark:text-neutral-100 truncate text-sm group-hover:text-accent transition-colors">
    ...
    <div className="overflow-y-auto px-3 py-4 sm:p-4 md:p-6 lg:p-8 max-w-6xl mx-auto h-full">
      ...
      <div className="flex items-center justify-between gap-3 mb-4 sm:mb-6">
        ...
        <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.96 }} transition={{ duration: 0.15 }}>
          <Link
            href="/app/new"
            className="inline-flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-accent text-white text-xs sm:text-sm font-medium hover:bg-accent/90 active:bg-accent/80 transition-colors min-h-[40px] sm:min-h-[44px] shrink-0 shadow-sm shadow-accent/25"
          >
      ...
      {projects.length === 0 ? (
        ...
        >
          ...
          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
            <Link
              href="/app/new"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-accent text-white text-sm font-medium hover:bg-accent/90 transition-colors min-h-[44px] shadow-sm shadow-accent/25"
            >
              <Plus className="w-4 h-4" />
