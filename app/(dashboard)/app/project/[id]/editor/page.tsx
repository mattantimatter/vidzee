...
function OverlayDisplay({
  ...
  switch (overlay.type) {
    case "intro":
      return (
        <div
          className={`${baseClass} inset-0 flex items-center justify-center`}
        >
          <div className="text-center">
            <div className="text-2xl md:text-3xl font-bold tracking-tight drop-shadow-lg">
              {overlay.text}
            </div>
          </div>
        </div>
    ...
    case "price_card":
      return (
        <div className={`${baseClass} top-4 right-4`}>
          <div className="bg-accent/90 backdrop-blur-sm rounded-xl px-5 py-3">
...
export default function EditorPage(): ReactNode {
  ...
  const loadData = useCallback(async () => {
    ...
    if (savedState?.clips && savedState.clips.length > 0) {
      ...
      // Build initial editor state from scenes + renders
      ...
      if (editorClips.length > 0 && proj) {
        ...
        // Lower third captions on remaining clips
        ...
        if (lastClip && proj.agent_name) {
          defaultOverlays.push({
            ...
            clipId: lastClip.id,
            type: "outro",
            text: [proj.agent_name, proj.brokerage]
              .filter(Boolean)
              .join(" · "),
            startSec: 0,
  ...
  const currentOverlay = useMemo(
    () =>
      overlays.find(
        (o) => clips[currentClipIndex]?.id === o.clipId
      ) ?? undefined,
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="text-center max-w-md">
            ...
            <p className="text-neutral-500 text-sm mb-6">
              Generate video clips from your storyboard first, then come back to
              edit them.
            </p>
            <Link
        ...
        <div className="flex-1 min-h-0 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="lg:flex-1 min-h-0 flex flex-col items-center justify-center bg-neutral-950 p-4 relative">
            <div className="relative w-full max-w-2xl aspect-video bg-black rounded-lg overflow-hidden">
              ...
              {/* Fade to black overlay for that transition */}
              {currentClip?.transition === "fade_black" &&
                transitionProgress < 0.5 && (
                  <div
                    className="absolute inset-0 bg-black"
                    style={{ opacity: 1 - transitionProgress * 2 }}
                  />
                )}
          ...
          <div className="w-full lg:w-80 shrink-0 border-t lg:border-t-0 lg:border-l border-neutral-200 bg-white overflow-y-auto">
            ...
            <div className="p-4 border-b border-neutral-100">
              ...
              <div className="space-y-3">
                ...
                {musicError && (
                  <p className="text-xs text-red-500">{musicError}</p>
                )}
                  ...
                  <div className="pt-2 border-t border-neutral-100">
                    ...
                    {overlays
                      ...
                      .map((overlay) => (
                        <div
                          key={overlay.id}
                          className="space-y-2 bg-neutral-50 rounded-lg p-2"
                        >
                          <div className="flex items-center gap-2">
                            <select
                              value={overlay.type}
                              onChange={(e) =>
                                updateOverlay(overlay.id, {
                                  type: e.target.value as OverlayType,
                                })
                              }
        ...
        <div className="shrink-0 border-t border-neutral-200 bg-neutral-50">
          ...
          >
            {clips.map((clip, index) => {
              ...
              const clipOverlay = overlays.find(
                (o) => o.clipId === clip.id
              );
              ...
              return (
                <div key={clip.id} className="flex items-center gap-1">
                  ...
                  {index > 0 && (
                    ...
                    >
                      <div
                        className={`w-4 h-4 rounded-full border-2 flex items-center justify-center text-[8px] font-bold transition-colors ${
                          clip.transition === "cut"
